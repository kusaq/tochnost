...
- **FR-23.** Сырые пакеты (`sensor_raw`) и `ingest_rejected` привязываются к РШР по снимку FSM **на момент приёма**, по тем же правилам, что и сам FSM: пост 1 → активная, пост 2 и Modbus → только `rail_at_post2` (без фолбэка на FIFO/активную). Привязка приблизительная и помечается `rshr_id_provisional`; UI рисует её как `~РШР N`. Фильтр по `rshr_id` ищет в обоих буферах (durable + firehose). → **INV-2**, **ADR-0012**
